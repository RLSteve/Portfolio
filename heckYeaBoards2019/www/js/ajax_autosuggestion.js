 ...
 */
var xmlHttp;
var numTitles = 0;  //total number of suggested board brands
var activeTitle = -1;  //board brand currently being selected
...
function suggest(query) {
    ...
    xmlHttp.onreadystatechange = function () {
        ...
        if (xmlHttp.readyState === 4 && xmlHttp.status === 200) {
            ...
            displayTitles(Array.from(new Set(titles)));
        }
    };
...
/* This function populates the suggestion box with spans containing all the brands
...
function displayTitles(titles) {
    ...
    var divContent = "";
    //retrieve the brands from the JSON doc and create a new span for each title
    for (i = 0; i < titles.length; i++) {
        divContent += "<span id=s_" + i + " onclick='clickTitle(this)'>" + titles[i] + "</span>";
        /*divContent = titles.filter(function(i, pos) {
            return titles.indexOf(i) == pos;
        });*/
    }
...
function clickTitle(titles) {
    //display the title in the search box
    searchBoxObj.value = titles.innerHTML;
